...
export default function TesteDesenhaMapa() {
   ...
   const convertByPoint = async (item) => await mapRef.current?.coordinateForPoint(item);
   ...
   const isVisiblePolygons = useMemo(() => isReady && polygon.polygons && polygon.polygons.length > 0, [isReady, polygon.polygons]);
